...
class CategoriesControllerTest < ActionController::TestCase
  ...
  def test_6_rjs_should_fail_on_create_with_duplicate_permalink
    xml_http_request :post, :create, { :category => { :name => "Files", :permalink => @main_category.permalink } }, { :admin_password => PASSWORD }
    savefile(@response.body)
    assert_template("new")
    assert_select_rjs 
  ...
  def test_7_rjs_should_fail_on_create_with_duplicate_name
    xml_http_request :post, :create, { :category => { :name => @main_category.name, :permalink => "file-handling-new" } }, { :admin_password => PASSWORD }
